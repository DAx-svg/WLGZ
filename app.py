...
def init_db():
    ...
    db.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS bulk_materials (
            id              INTEGER PRIMARY KEY AUTOINCREMENT,
            name            TEXT NOT NULL,
            spec            TEXT DEFAULT '',
        ...
        CREATE TABLE IF NOT EXISTS bulk_records (
            ...
            purpose         TEXT DEFAULT '',
            related_sn      TEXT DEFAULT '',
            created_time    TEXT NOT NULL,
            FOREIGN KEY (material_id) REFERENCES bulk_materials(id) ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS idx_bulk_records_material ON bulk_records(material_id);
    """)
...
BULK_CATEGORIES = ['紧固件', '结构件', '电气件', '管材', '板材', '其他']
...
def bulk_page():
    ...
    if search:
        materials = db.execute(
            "SELECT * FROM bulk_materials WHERE name LIKE ? OR spec LIKE ? OR model LIKE ? "
            "ORDER BY name",
            (f'%{search}%', f'%{search}%', f'%{search}%')
        ).fetchall()
    elif filter_cat:
        materials = db.execute(
            "SELECT * FROM bulk_materials WHERE category = ? ORDER BY name",
            (filter_cat,)
        ).fetchall()
    else:
        materials = db.execute(
            "SELECT * FROM bulk_materials ORDER BY name"
        ).fetchall()

    # 统计
    stats = {
        'total': db.execute("SELECT COUNT(*) AS cnt FROM bulk_materials").fetchone()['cnt'],
        'total_qty': db.execute("SELECT SUM(quantity) AS s FROM bulk_materials").fetchone()['s'] or 0,
        'low_stock': db.execute(
            "SELECT COUNT(*) AS cnt FROM bulk_materials WHERE quantity <= min_stock AND min_stock > 0"
        ).fetchone()['cnt'],
        'categories': BULK_CATEGORIES,
    }
...
def api_bulk_add():
    ...
    now = datetime.now(ZoneInfo('Asia/Shanghai')).strftime('%Y-%m-%d %H:%M:%S')
    db.execute(
        "INSERT INTO bulk_materials (name, spec, category, model, quantity, unit, "
        "supplier, location, min_stock, first_inbound, remarks) "
        "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
        (name, data.get('spec', ''), data.get('category', '其他'),
         data.get('model', ''), data.get('initial_qty', 0), data.get('unit', '个'),
         data.get('supplier', ''), data.get('location', ''),
         data.get('min_stock', 0), now, data.get('remarks', ''))
    )
    mid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
    if data.get('initial_qty', 0) > 0:
        db.execute(
            "INSERT INTO bulk_records (material_id, type, quantity, operator_name, "
            "purpose, created_time) VALUES (?,?,?,?,?,?)",
            (mid, '入库', data['initial_qty'], data.get('operator', ''), '首次入库', now)
        )
    db.commit()
    return jsonify({'success': True, 'message': f'来料"{name}"已添加'})


@app.route('/api/bulk/<int:mid>', methods=['PUT'])
def api_bulk_edit(mid):
    """编辑来料信息"""
    db = get_db()
    mat = db.execute("SELECT * FROM bulk_materials WHERE id=?", (mid,)).fetchone()
    if not mat:
        return jsonify({'success': False, 'message': '物料不存在'}), 404
    data = request.get_json(force=True)
    db.execute(
        "UPDATE bulk_materials SET name=?, spec=?, category=?, model=?, unit=?, "
        "supplier=?, location=?, min_stock=?, remarks=? WHERE id=?",
        (data.get('name', mat['name']), data.get('spec', mat['spec']),
         data.get('category', mat['category']), data.get('model', mat['model']),
         data.get('unit', mat['unit']), data.get('supplier', mat['supplier']),
         data.get('location', mat['location']), data.get('min_stock', mat['min_stock']),
         data.get('remarks', mat['remarks']), mid)
    )
    db.commit()
    return jsonify({'success': True, 'message': '来料信息已更新'})


@app.route('/api/bulk/<int:mid>/inbound', methods=['POST'])
def api_bulk_inbound(mid):
    """来料入库"""
    db = get_db()
    mat = db.execute("SELECT * FROM bulk_materials WHERE id=?", (mid,)).fetchone()
    if not mat:
    ...
    db.execute(
        "INSERT INTO bulk_records (material_id, type, quantity, operator_name, "
        "purpose, created_time) VALUES (?,?,?,?,?,?)",
        (mid, '入库', qty, data.get('operator', ''), data.get('purpose', ''), now)
    )
    db.execute("UPDATE bulk_materials SET quantity = quantity + ? WHERE id=?", (qty, mid))
    if not mat['first_inbound']:
        db.execute("UPDATE bulk_materials SET first_inbound = ? WHERE id=?", (now, mid))
    db.commit()
    return jsonify({'success': True, 'message': f'入库 {qty}{mat["unit"]}，当前库存 {mat["quantity"] + qty}{mat["unit"]}'})


@app.route('/api/bulk/<int:mid>/outbound', methods=['POST'])
def api_bulk_outbound(mid):
    """来料出库（领用）"""
    db = get_db()
    mat = db.execute("SELECT * FROM bulk_materials WHERE id=?", (mid,)).fetchone()
    if not mat:
    ...
    if qty <= 0:
        return jsonify({'success': False, 'message': '数量必须大于0'}), 400
    if qty > mat['quantity']:
        return jsonify({'success': False, 'message': f'库存不足，当前仅 {mat["quantity"]}{mat["unit"]}'}), 400
    now = datetime.now(ZoneInfo('Asia/Shanghai')).strftime('%Y-%m-%d %H:%M:%S')
    db.execute(
        "INSERT INTO bulk_records (material_id, type, quantity, operator_name, "
        "purpose, related_sn, created_time) VALUES (?,?,?,?,?,?,?)",
        (mid, '出库', qty, data.get('operator', ''), data.get('purpose', ''),
         data.get('related_sn', ''), now)
    )
    db.execute("UPDATE bulk_materials SET quantity = quantity - ?, last_outbound = ? WHERE id=?",
               (qty, now, mid))
    db.commit()
    return jsonify({'success': True, 'message': f'出库 {qty}{mat["unit"]}，剩余 {mat["quantity"] - qty}{mat["unit"]}'})


@app.route('/api/bulk/<int:mid>/records')
def api_bulk_records(mid):
    """获取来料出入库流水"""
    db = get_db()
    records = db.execute(
        "SELECT * FROM bulk_records WHERE material_id = ? ORDER BY created_time DESC LIMIT 50",
        (mid,)
    ).fetchall()
    return jsonify([{
        'id': r['id'], 'type': r['type'], 'quantity': r['quantity'],
        'operator_name': r['operator_name'], 'purpose': r['purpose'],
        'related_sn': r['related_sn'], 'created_time': r['created_time']
    } for r in records])


@app.route('/api/bulk/<int:mid>', methods=['DELETE'])
def api_bulk_delete(mid):
    """删除来料及其所有记录"""
    db = get_db()
    if not db.execute("SELECT id FROM bulk_materials WHERE id=?", (mid,)).fetchone():
        return jsonify({'success': False, 'message': '物料不存在'}), 404
    db.execute("DELETE FROM bulk_records WHERE material_id=?", (mid,))
    db.execute("DELETE FROM bulk_materials WHERE id=?", (mid,))
    db.commit()
